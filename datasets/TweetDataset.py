...
class TweetDataset(Dataset):
    ...
    # Constructor
    def __init__(self, root='./data', download=True, lang='en', text_transform=None):
        """
        Constructor
        ...
        self.gender_classes = {'female': 0, 'male': 1}
        self.country_classes = {'canada': 0, 'australia': 1, 'new zealand': 2, 'ireland': 3, 'great britain': 4, 'united states': 5}
        self.max = 0
        self.input_dim = 0
    ...
    def __getitem__(self, item):
        ...
        # Labels
        # labels = torch.LongTensor(tweets.size(0), 2).fill_(self.labels[current_idxs][0])
        labels = torch.LongTensor(tweets.size(0), 2).fill_(self.labels[current_idxs][0])

        # Fill
        labels[:, 0] = self.labels[current_idxs][0]
        labels[:, 1] = self.labels[current_idxs][1]
    ...
    def _load_labels(self):
        ...
        for line in label_file.split("\n"):
            if len(line) > 0:
                # ID and label
                idx, gender, country = line.split(":::")
                idx_to_labels[idx] = (self.gender_classes[gender], self.country_classes[country])
            # end if
        # end for
